...
本文档记录项目自动生成美术资源的风格规范。**图片类美术资源仅使用 AliyunBailianMCP_Wan26Media（`modelstudio_wanx26_image_generation`）生成**，生成时必须遵循本文档的规格与描述词要求，保证新生成资源与现有资产的一致性。

---

## 一、参考资产（Wan26Media 已生成）
...
| **其他** | 完全透明背景（无背景）、无阴影；引用见 `weapon_defs.gd` 的 `icon_path`。游戏中武器环显示时不得遮盖角色，见实现说明（player.gd 中武器图标 z_index） |
| **朝向规定** | 为便于游戏内使把手朝向玩家，**把手应位于图片的同一固定边**（推荐**下方**，即纹理 +Y 方向）。生成或绘制武器图标时保持该约定；代码中通过固定旋转（player.gd 中 `icon.rotation = PI/2`）使把手指向玩家中心。 |
...
## 三、Wan26Media 生成参数（武器/道具/魔法/元素图标）

当前**仅使用 AliyunBailianMCP_Wan26Media** 生成图片。调用 `modelstudio_wanx26_image_generation` 时须遵循本文档：

| 参数 | 建议 | 说明 |
|------|------|------|
| prompt | 见「四、描述词规范」 | 正向提示词，描述期望画面；**必须包含**视角（如 top-down）、风格（pixel art icon）及 **transparent background**。武器与角色类资源须包含 **no background, no shadow**（或 fully transparent background, no shadow） |
...
| 类别 | 示例描述词 |
|------|------------|
| 武器 | `short sword, top-down view, pixel art icon, game UI, fully transparent background, no background, no shadow` |
...
- 描述词使用英文；**必须包含 transparent background** 以要求透明背景；武器与角色须加 **no background, no shadow**
- 保持简洁，避免过长
- 与参考资产的描述词风格一致
...
- **游戏中武器与角色**：武器图标在武器环上显示时，通过 `player.gd` 中为武器图标设置 `z_index` 低于角色精灵，使武器绘于角色之后，避免遮盖人物。
...
3. **调用** **AliyunBailianMCP_Wan26Media** 的 `modelstudio_wanx26_image_generation` 工具：`prompt` 按描述词规范，`size` 按规格（wan2.6 支持 768×768～1440×1440，可用 1024*1024；需 96×96 时生成后须缩放），`n` 按需
...
**失败处理**：若 Wan26Media 调用失败，**仅提示哪些资产生成失败**，不调用其他图生接口（如 Pixellab、ali-image、GenerateImage 等）替代生成。

### 6.1 Pixellab（可选，非当前图生工具）

项目规定图片美术资源**仅使用 AliyunBailianMCP_Wan26Media** 生成，以下仅供了解。Pixellab 支持账号登录，使用 API Token 可提高配额、减少限流：
...
### 6.2 Wan26Media（当前图生工具）

**图片美术资源仅使用 AliyunBailianMCP_Wan26Media**，工具名为 `modelstudio_wanx26_image_generation`。生成前须阅读本文档规格与描述词，生成后保存到规定路径并满足尺寸与风格要求。

### 6.3 ali-image（阿里云 DashScope / FLUX，非默认图生工具）

项目内 **ali-image** MCP 使用 [ali-flux-mcp](https://github.com/echozyr2001/ali-flux-mcp) 本地部署；按规定**不**用于替代 Wan26Media 做美术资产生成。
...
- 新增 Wan26Media 生成资产后，若风格有显著变化，可更新本文档「参考资产」与「视觉特征」
- 参数或工具变更时同步更新「三、Wan26Media 生成参数」与「六、生成流程」
