...
| 背包面板 | 暂停菜单内背包 Tab，双独立面板：ContentPanel（背包内容：武器/魔法/道具网格）+ DetailPanel（物品详情）；点击或悬浮槽位在右侧显示详情 | pause_menu | BackpackPanel |
| 背包内容面板 | 背包左侧 ContentPanel，含武器/魔法/道具三区网格 | BackpackPanel | ContentPanel |
| 背包详情面板 | 背包右侧 DetailPanel，展示选中/悬浮槽位的名称、词条、效果、套装 2/4/6 件、售卖/合成按钮 | BackpackPanel | DetailPanel |
